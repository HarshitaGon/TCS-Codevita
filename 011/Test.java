/*
    ...
    of curtains in each box. The box with the maximum number of ‘aqua’ (a) color curtains
    is labeled. The task here is to find the number of ‘aqua’ color curtains in the labeled box.
    ...
    The output should be a positive integer number of print the message(if any) given
...
public class Test {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < str.length(); i += L) {
            String chunks = str.substring(i, Math.min(i + L, str.length()));

            int countA = 0;
            for (int j = 0; j < chunks.length(); ++j) {
                if (chunks.charAt(j) == 'a')
                    ++countA;
            }
...
/*
    ...
    -> handle edge case (leftover characters)

    * poore question ka sabse important part:-

        { chunk = str.substring(i, Math.min(i + L, str.length())); }

        Java substring(start, end) rule:
        start → included
        end → excluded
        end kabhi bhi string length se bada nahi ho sakta, warna error aata hai ❌
